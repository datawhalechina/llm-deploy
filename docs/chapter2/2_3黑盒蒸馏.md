...
黑盒蒸馏意味着教师模型的输出是我们唯一能获取到的训练资源，所以黑盒蒸馏整体的思路分成两步：
1. 从教师模型收集问答数据 
2. 使用这些数据对学生模型进行微调。
   
这份教程会详细介绍三种涌现能力的主流蒸馏方式，以及分别介绍针对第一步和第二步的改进方向。
值得一提的是，黑盒蒸馏和监督微调很相似，所以读者最好了解监督微调的概念。
...
这一部分将介绍一系列的损失函数，需要具备条件概率、熵和交叉熵相关的知识。
...
即通过对比学生的回答和老师的回答，老师可以发现困难的知识，并加生成更多困难知识的供学生学习。具体的细节可以查看[]
